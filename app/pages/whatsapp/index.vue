<template>
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-bold text-gray-900">WhatsApp Integration</h1>
        <p class="text-sm text-gray-500 mt-0.5">Manage your WhatsApp Business phone numbers</p>
      </div>
      ...
      <div v-for="inbox in inboxes" :key="inbox.id" class="card p-5 space-y-4">
        ...
        <div class="flex items-start justify-between">
          ...
          <button
            ...
            :class="inbox.isActive ? 'bg-green-500' : 'bg-gray-200'"
        ...
        <div
          ...
          :class="inbox.isActive ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50'"
        >
          <div class="flex items-center gap-1.5">
            ...
            <span
              ...
              :class="inbox.isActive ? 'text-green-700' : 'text-gray-500'"
            >
    ...
    <div v-if="webhookInfo" class="card p-5 space-y-4">
      <h2 class="text-sm font-semibold text-gray-700">Webhook Configuration</h2>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="space-y-1">
          <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">Callback URL</p>
          <p
            class="text-sm font-mono bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 break-all"
          >
            {{ webhookInfo.callbackUrl }}
          </p>
        </div>
        <div class="space-y-1">
          <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">Verify Token</p>
          <p class="text-sm font-mono bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
      ...
      <div>
        <p class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Setup Steps</p>
        <ol class="space-y-2">
          <li
            v-for="(step, i) in webhookInfo.setupSteps"
            :key="i"
            class="flex gap-2.5 text-sm text-gray-600"
          >
            <span
              class="flex-shrink-0 w-5 h-5 rounded-full bg-green-100 text-green-700 text-xs font-bold flex items-center justify-center mt-0.5"
              >{{ i + 1 }}</span
            >
            <span>{{ step }}</span>
          </li>
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
          ...
          >
            <DialogPanel class="card p-6 w-full max-w-md shadow-xl">
              ...
              <div
                class="rounded-lg bg-green-50 border border-green-200 p-3 mb-4 text-sm text-green-700"
              >
