<template>
  <div class="p-6">
    <h1 class="text-xl font-bold text-gray-900 mb-6">Contacts</h1>
    ...
    <div v-else-if="contacts.length === 0" class="card p-10 text-center text-gray-400 text-sm">
      No contacts found.
    </div>
          ...
          >
            <td class="px-4 py-3">
              <div class="flex items-center gap-3">
                <div
                  class="w-8 h-8 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-medium text-sm"
                >
                  {{ (c.contactName || c.contactId || '?').charAt(0).toUpperCase() }}
                </div>
                <div>
                  <div class="font-medium text-gray-900">{{ c.contactName || '—' }}</div>
                  <div class="text-xs text-gray-400">{{ c.contactId }}</div>
                </div>
              </div>
            </td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3 text-gray-700">{{ c.conversationCount }}</td>
            <td class="px-4 py-3 text-gray-500">{{ formatDate(c.lastActivity) }}</td>
